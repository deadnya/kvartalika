...
import {VariantCard} from "../components/VariantCard.tsx";
...
const ApartmentPage = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (flat) {
                ...
                setSelectedHome(
                    homes.find((home) => flat.flat.homeId === home.id) ?? null,
                );
            } else {
    ...
    useEffect(() => {
        if (selectedFlat?.flat) setVariantsLoading(false);
    }, [selectedFlat]);
    ...
    let images = useDecoration
        ? safeImage(selectedFlat.imagesResolved, "flat")
        : safeImage(selectedFlat.imagesCleanResolved, "flat");
    ...
    const filteredVariants =
        selectedFlat.flat.variants?.filter(
            (v) => v.hasDecoration === useDecoration,
        ) ?? [];
    const placeholderText = `Сейчас нет доступных квартир ${
    ...
    return (
        <div className="min-h-screen pt-20">
                        ...
                        <div className="mb-4">
                            <div className="relative rounded-xl overflow-hidden bg-gray-200 w-full">
                                ...
                                {selectedFlat.categories?.some((el) =>
                                    ...
                                    <div
                                        className="absolute bottom-2 left-4 bg-red-600 text-white px-2 py-2 rounded-lg text-xs font-semibold flex items-center gap-1">
          ...
          {selectedFlat.flat?.numberOfRooms ?? 1}
        </span>
                                    </div>
                                    <div className="flex justify-between py-3 border-b border-gray-200">
                                        <span className="text-gray-600">Количество санузлов:</span>
                                        <span className="font-medium">
          {selectedFlat.flat?.numberOfBathrooms ?? 1}
        </span>
                                    </div>
                                    <div className="flex justify-between py-3 border-b border-gray-200">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between py-3 border-b border-gray-200 items-center">
                                        ...
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            ...
                                            />
                                            <div
                                                className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-blue-500 transition"></div>
                                            <span
                                                className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full peer-checked:translate-x-5 transition"></span>
            ...
            {useDecoration ? "С отделкой" : "Без отделки"}
                                    ...
                                    <div className="flex justify-between py-3 border-b border-gray-200">
                                        ...
                                        <span className="font-medium">
          {selectedFlat.flat.address ?? "Нужно уточнить"}
        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {selectedFlat.flat.latitude &&
                            selectedFlat.flat.longitude &&
                            selectedFlat.flat.address && (
                                <div className="mt-8">
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24">
                            <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
                                ...
                                {/*    <span className="font-medium">*/}
                                {/*      {selectedFlat.flat.area ?? "Неизвестно"} м²*/}
                                {/*    </span>*/}
                                {/*  </div>*/}
                                {/*  <div className="flex items-center justify-between text-sm">*/}
                                {/*    <span className="text-gray-600">Комнат:</span>*/}
                                ...
                                <button
                                    ...
                                    aria-label="Записаться на просмотр квартиры"
                                    className="sign-up-button w-full bg-primary-600 text-surface-50 py-4 rounded-xl font-semibold text-lg hover:bg-primary-700 transition-all duration-200 shadow-lg hover:shadow-xl mb-4"
                                >
                                    Записаться на просмотр
                                </button>
                            </div>

                            <div className="lg:col-span-1 variants-list">
                                {variantsLoading ? (
                                    <div className="flex justify-center py-8">
                                        <div
                                            className="w-8 h-8 rounded-full border-2 border-primary-600 border-t-transparent animate-spin"/>
                                    </div>
                                ) : filteredVariants.length === 0 ? (
                                    <div
                                        className="py-6 px-4 text-center text-sm text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                        {placeholderText}
                                    </div>
                                ) : (
