...
import SearchBarLayouts from "../components/SearchBarLayouts.tsx";
import LayoutsCard from "../components/LayoutsCard.tsx";
import { useLayoutsStore } from "../store/layouts.store.ts";
...
const LayoutsPage = () => {
  ...
  const pagedLayouts = useMemo(() => {
    ...
    return layouts.slice(start, start + limit);
  }, [layouts, currentPage, limit]);

  const getPageTitle = useCallback(() => {
    const curCat = currentSearchParams.categoriesId;
    if (curCat && curCat.length === 1) {
      const cat = categories?.find((c) => c.id === curCat[0]);
      if (cat) return `Планировки категории ${cat.name}`;
    }
    if (currentSearchParams.homeId) {
      const curHome = homes.find((c) => c.id === currentSearchParams.homeId);
      if (curHome) return `Планировки в комплексе ${curHome.name}`;
    }
    return "Планировки на любой вкус";
  }, [currentSearchParams, categories, homes]);

  const getPageDescription = useCallback(() => {
    if (
      currentSearchParams.categoriesId &&
      currentSearchParams.categoriesId.length === 1
    ) {
      const curCat = currentSearchParams.categoriesId;
      const cat = categories?.find((c) => c.id === curCat[0]);
      if (cat) return `Подборка планировок из категории ${cat.name}`;
    }
    if (currentSearchParams.rooms) {
      return `Подберите идеальную ${currentSearchParams.rooms}-комнатную планировку`;
    }
    if (currentSearchParams.homeId) {
      return `Большой выбор планировок в выбранном комплексе`;
    }
    return "Большой выбор планировок в лучших жилых комплексах города";
      ...
      <section className="bg-gradient-to-r from-primary-600 to-primary-800 via-primary-700 py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center text-white mb-12">
            ...
            <p className="text-xl text-gray-200 max-w-3xl mx-auto animate-slide-up">
              {pageDescription}
            </p>
          </div>
      ...
      <div className="container mx-auto px-4 py-12">
        <nav className="mb-8">
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="text-gray-900">Планировки</span>
          </div>
        </nav>
        ...
        {!isSearching && layouts.length === 0 && !searchError && (
        ...
        {pagedLayouts.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {pagedLayouts.map((apartment) => (
              <LayoutsCard
                homeName={
                  homes.find((home) => home.id === apartment.homeId)?.name ??
                  "?"
                }
                key={apartment.id}
                layout={apartment}
        ...
        <section className="mt-20 bg-gradient-to-br from-primary-600 via-primary-700 to-primary-800 rounded-3xl p-12 relative overflow-hidden">
          ...
          <div className="max-w-3xl mx-auto text-center relative z-10">
            ...
            <p className="text-gray-200 mb-8 text-lg">
              Наши специалисты помогут подобрать идеальный вариант под ваши
              требования и бюджет
            </p>
            <button
