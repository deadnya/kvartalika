...
export class ContentService {
  ...
  static async listDirectories(): Promise<DirectoriesResponse> {
    return ContentService.safeRequest(() =>
      apiClient.get<DirectoriesResponse>("/directories"),
    );
  }

  static async getDirectory(
    pathParts: string[],
  ): Promise<DirectoriesChildrenResponse> {
    const path = pathParts.map(encodeURIComponent).join("/");
    return ContentService.safeRequest(() =>
      apiClient.get<DirectoriesChildrenResponse>(`/directories/${path}`),
    );
  }
  ...
  static async createDirectory(pathParts: string[]): Promise<void> {
    const path = pathParts.map(encodeURIComponent).join("/");
    return ContentService.safeRequest(() =>
      apiClient.post(`/directories/${path}`),
    );
  }
  ...
  static async deleteDirectory(pathParts: string[]): Promise<void> {
    const path = pathParts.map(encodeURIComponent).join("/");
    return ContentService.safeRequest(() =>
      apiClient.delete(`/directories/${path}`),
    );
  }
  ...
  static async listFilesInDir(dirParts: string[]): Promise<FilesResponse> {
    ...
    return ContentService.safeRequest(() =>
      apiClient.get<FilesResponse>(`/files/list/${dir}`),
    );
  }
  ...
  static async getFile(pathParts: string[]): Promise<Blob> {
    const path = pathParts.map(encodeURIComponent).join("/");
    try {
      const resp = await apiClient.get<Blob>(`/files/${path}`, {
        responseType: "blob",
      });
      return resp.data;
    } catch (err) {
  ...
  static async uploadFile(dirParts: string[], file: File): Promise<void> {
    ...
    return ContentService.safeRequest(() =>
      apiClient.postForm(`/files/upload/${dir}`, form),
    );
  }
  ...
  static async updateFlat(
    id: number,
    data: FlatWithCategoryRequest,
  ): Promise<void> {
    return ContentService.safeRequest(
  ...
  static async addCategoryToFlat(
    flatId: number,
    categoryId: number,
  ): Promise<void> {
    return ContentService.safeRequest(() =>
  ...
  static async removeCategoryFromFlat(
    flatId: number,
    categoryId: number,
  ): Promise<void> {
    return ContentService.safeRequest(() =>
  ...
  // Layouts management
  static async createLayout(data: LayoutRequest): Promise<void> {
    return ContentService.safeRequest(() =>
      apiClient.post<void, LayoutRequest>("/layouts", data),
    );
  }

  static async updateLayout(id: number, data: LayoutRequest): Promise<void> {
    return ContentService.safeRequest(
      () => apiClient.put<void, HomeRequest>(`/layouts/${id}`, data),
      "Layout not found",
      404,
    );
  }
  ...
  static async bulkDeletePhotos(
    photoIds: number[],
  ): Promise<ApiResponse<void>[]> {
    return Promise.all(photoIds.map((id) => this.deletePhoto(id)));
  }

  // Flat Variants Management
  static async getAllVariants(): Promise<FlatVariantDto[]> {
    return ContentService.safeRequest(() =>
      apiClient.get<FlatVariantDto[]>("/flats/variants"),
    );
  }

  static async createVariant(data: CreateFlatVariantRequest): Promise<void> {
    return ContentService.safeRequest(() =>
      apiClient.post<void, CreateFlatVariantRequest>("/flats/variants", data),
    );
  }

  static async updateVariant(data: UpdateFlatVariantRequest): Promise<void> {
    return ContentService.safeRequest(
      () => apiClient.put<void, UpdateFlatVariantRequest>("/flats/variants", data),
      "Variant not found",
      404,
    );
  }
  ...
  static async deleteVariant(id: number): Promise<void> {
    return ContentService.safeRequest(
      () => apiClient.delete<void>(`/flats/variants/${id}`),
      "Variant not found",
      404,
    );
  }
