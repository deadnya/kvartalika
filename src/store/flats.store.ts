...
export const useFlatsStore = create<flatsState & flatsActions>()(
  persist(
    (set, get) => ({
      flats: [],
      homes: [],
        ...
        try {
          ...
          set((state) => {
            ...
            resolved.forEach((flat) => {
              const existingIndex = updatedFlats.findIndex(
                (h) => h.flat.id === flat.flat.id,
              );
              if (existingIndex >= 0) {
      ...
      setSelectedLayout: (layout: Layout | null) => {
